...
const countVisible = grid => {
  ...
  for (let i = 0; i < grid.length; i++) {
    for (let j = 0; j < grid[0].length; j++) {
      ...
      let u = i - 1
      let d = i + 1
      while(l > 0 && grid[i][l] < grid[i][j]) { 
        leftScore++
        l--
      }
      while(r < grid[0].length - 1  && grid[i][r] < grid[i][j]) {
        rightScore++
        r++
      } 

      while (u > 0 && grid[u][j] < grid[i][j]) {
        upScore++
        u--
      }
      while (d < grid.length - 1 && grid[d][j] < grid[i][j]) {
        downScore++
        d++
      }

      //scores.push([leftScore , upScore, rightScore, downScore])
      scores.push(upScore * downScore * leftScore * rightScore)
  ...
  //return scores
  return scores.reduce((a, e) => Math.max(a, e), -1) 
}
compose(
  log,
  countVisible,
